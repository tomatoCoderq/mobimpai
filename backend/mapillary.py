...
def score_mapillary(
    ...
    zoom is accepted only for a compatible signature with score_tile and cache
    keys; Mapillary does not use it. Returns (score, stats) or (None, stats)
    when there is no imagery near the coordinate.
    """
    ...
    if DEBUG_SAVE and debug_tag:
        _save_snapshot(image, pred, lng, lat, zoom, f"mly_{debug_tag}")
